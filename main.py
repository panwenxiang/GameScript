...
for imgName in os.listdir(batchImportPath):
    ...
    scale = [1]
    for sl in scale:
        scaledTemplateImg = cv2.resize(imgTemplateReadGray, (int(ww1 * sl), int(hh1 * sl)))
        ...
        # 模板匹配
        match = cv2.matchTemplate(imgBottomGray, scaledTemplateImg, cv2.TM_CCOEFF_NORMED)
        # cv2.imshow('2', scaledTemplateImg)
        # 查看下测试比例
        locations = np.where(match >= 0)

        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(match)
        print(imgName, '  缩放比例：', str(sl), '  最高匹配度', max_val)
        if max_val > 0.9:
            # 匹配度小数转成百分比
            matchPercent = "%.1f%%" % (max_val * 100)
            cv2.rectangle(imgBottom, max_loc, (max_loc[0] + imgW, max_loc[1] + imgH), (0, 255, 0), 2)  # 画方框，图片/初始坐标/结束坐标/rgb/粗细
